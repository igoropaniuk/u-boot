...
#define PDO_BATT_MIN_VOLT(mv) \
	((((mv) / 50) & PDO_VOLT_MASK) << PDO_BATT_MIN_VOLT_SHIFT)
#define PDO_BATT_MAX_VOLT(mv) \
	((((mv) / 50) & PDO_VOLT_MASK) << PDO_BATT_MAX_VOLT_SHIFT)
...
#define PDO_VAR_MIN_VOLT(mv) \
	((((mv) / 50) & PDO_VOLT_MASK) << PDO_VAR_MIN_VOLT_SHIFT)
#define PDO_VAR_MAX_VOLT(mv) \
	((((mv) / 50) & PDO_VOLT_MASK) << PDO_VAR_MAX_VOLT_SHIFT)
#define PDO_VAR_MAX_CURR(ma) \
	((((ma) / 10) & PDO_CURR_MASK) << PDO_VAR_MAX_CURR_SHIFT)
...
#define PDO_FIXED_OP_CURR(ma) \
	((((ma) / 10) & RDO_CURR_MASK) << RDO_FIXED_OP_CURR_SHIFT)
#define PDO_FIXED_MAX_CURR(ma) \
	((((ma) / 10) & RDO_CURR_MASK) << RDO_FIXED_MAX_CURR_SHIFT)
...
#define RDO_BATT_OP_PWR(mw) \
	((((mw) / 250) & RDO_PWR_MASK) << RDO_BATT_OP_PWR_SHIFT)
#define RDO_BATT_MAX_PWR(mw) \
	((((mw) / 250) & RDO_PWR_MASK) << RDO_BATT_MAX_PWR_SHIFT)
...
int tcpc_get_cc_status(struct tcpc_port *port,
		       enum typec_cc_polarity *polarity,
...
int tcpc_polling_reg(struct tcpc_port *port, uint8_t reg,
		     uint8_t reg_width, uint16_t mask,
		     uint16_t value, ulong timeout_ms);
...
int tcpc_init(struct tcpc_port *port,
	      struct tcpc_port_config config, ss_mux_sel ss_sel_func);
