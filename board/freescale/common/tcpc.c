...
int tcpc_set_plug_orientation(struct tcpc_port *port,
		       ...
		       enum typec_cc_state *state)
{
	...
	err = dm_i2c_read(port->i2c_dev, TCPC_CC_STATUS,
			  (uint8_t *)&valb_cc, 1);
	if (err) {
		tcpc_log(port,
			 "%s dm_i2c_read failed, err %d\n", __func__, err);
			...
			case 0x1:
				if (cc1 == 0x1) {
					*state = TYPEC_STATE_SRC_BOTH_RA;
					tcpc_log(port,
						 "SRC.Ra on both CC1 and CC2\n");
				} else if (cc1 == 0x2) {
					*state = TYPEC_STATE_SRC_RD_RA;
					tcpc_log(port,
						 "SRC.Ra on CC2, SRC.Rd on CC1\n");
				} else if (cc1 == 0x0) {
...
int tcpc_clear_alert(struct tcpc_port *port, uint16_t clear_mask)
{
	...
	err = dm_i2c_write(port->i2c_dev, TCPC_ALERT,
			   (const uint8_t *)&clear_mask, 2);
	if (err) {
		tcpc_log(port,
			 "%s dm_i2c_write failed, err %d\n", __func__, err);
		return -EIO;
...
int tcpc_send_command(struct tcpc_port *port, uint8_t command)
{
	...
	err = dm_i2c_write(port->i2c_dev, TCPC_COMMAND,
...
int tcpc_polling_reg(struct tcpc_port *port, uint8_t reg, uint8_t reg_width,
		     uint16_t mask, uint16_t value, ulong timeout_ms)
{
	uint16_t val = 0;
	int err;
	ulong start;
	...
	do {
		err = dm_i2c_read(port->i2c_dev, reg,
				  (uint8_t *)&val, reg_width);
		if (err)
			return -EIO;
...
int tcpc_setup_dfp_mode(struct tcpc_port *port)
{
	...
	/* At least wait tCcStatusDelay + tTCPCFilter + tCcTCPCSampleRate (max)
	 * = 200us + 500us + ?ms PTN5110 datasheet does not contain the sample
	 * rate value, according other productions, the sample rate is at ms
	 * level, about 2 ms -10ms. So wait 100ms should be enough.
	 */
	mdelay(100);
	...
	if (ret) {
		tcpc_log(port,
			 "%s: Polling ALERT register, TCPC_ALERT_CC_STATUS bit failed, ret = %d\n",
			 __func__, ret);
		return ret;
	}
	...
	if (!ret) {
		...
		if (state != TYPEC_STATE_SRC_RD_RA &&
		    state != TYPEC_STATE_SRC_RD)
			return -EPERM;
...
int tcpc_setup_ufp_mode(struct tcpc_port *port)
{
	...
	if (!tcpc_pd_sink_check_charging(port)) {
		...
		/* At least wait tCcStatusDelay + tTCPCFilter +
		 * tCcTCPCSampleRate(max) = 200us + 500us + ?ms.
		...
		ret = tcpc_polling_reg(port, TCPC_ALERT, 2,
...
static int tcpc_pd_receive_message(struct tcpc_port *port,
				   struct pd_message *msg)
{
	...
	/* Generally the max tSenderResponse is 30ms, max
	 * tTypeCSendSourceCap is 200ms, we set the timeout to 500ms
	 */
	ret = tcpc_polling_reg(port, TCPC_ALERT, 2, TCPC_ALERT_RX_STATUS,
			       TCPC_ALERT_RX_STATUS, 500);
	if (ret) {
		tcpc_log(port,
			 "%s: Polling ALERT register, TCPC_ALERT_RX_STATUS bit failed, ret = %d\n",
			 __func__, ret);
		return ret;
	}
	...
	if (cnt > 0) {
		ret = dm_i2c_read(port->i2c_dev, TCPC_RX_BUF_FRAME_TYPE,
				  (uint8_t *)msg, cnt);
		if (ret)
			return -EIO;
		...
		ret = dm_i2c_write(port->i2c_dev, TCPC_ALERT,
				   (const uint8_t *)&val, 2);
		if (ret)
			return -EIO;
	}
...
static int tcpc_pd_transmit_message(struct tcpc_port *port,
				    struct pd_message *msg_p, uint8_t bytes)
{
	int ret;
	uint8_t valb;
	uint16_t val;
	...
	ret = dm_i2c_write(port->i2c_dev, TCPC_TX_BYTE_CNT,
			   (const uint8_t *)&bytes, 1);
	if (ret)
		return -EIO;

	ret = dm_i2c_write(port->i2c_dev, TCPC_TX_HDR,
			   (const uint8_t *)&(msg_p->header), bytes);
	if (ret)
		return -EIO;

	valb = (3 << TCPC_TRANSMIT_RETRY_SHIFT)
		| (TCPC_TX_SOP << TCPC_TRANSMIT_TYPE_SHIFT);
	ret = dm_i2c_write(port->i2c_dev, TCPC_TRANSMIT,
			   (const uint8_t *)&valb, 1);
	if (ret)
		return -EIO;
	...
	if (ret) {
		if (ret == -ETIME) {
			ret = dm_i2c_read(port->i2c_dev, TCPC_ALERT,
					  (uint8_t *)&val, 2);
			if (ret)
				return -EIO;
			...
			if (val & TCPC_ALERT_TX_FAILED)
				tcpc_log(port,
					 "%s: PD TX FAILED, ALERT = 0x%x\n",
					 __func__, val);

			if (val & TCPC_ALERT_TX_DISCARDED)
				tcpc_log(port,
					 "%s: PD TX DISCARDED, ALERT = 0x%x\n",
					 __func__, val);

		} else {
			tcpc_log(port,
				 "%s: Polling ALERT register, TCPC_ALERT_TX_SUCCESS bit failed, ret = %d\n",
				 __func__, ret);
		}
	} else {
	...
	/* Clear ALERT status */
	val &= (TCPC_ALERT_TX_FAILED | TCPC_ALERT_TX_DISCARDED
...
static void tcpc_log_source_caps(struct tcpc_port *port, uint32_t *caps,
...
static int tcpc_pd_select_pdo(uint32_t *caps, uint32_t capcount,
			      uint32_t max_snk_mv, uint32_t max_snk_ma)
{
...
static void tcpc_pd_sink_process(struct tcpc_port *port)
{
	...
	while (tcpc_pd_receive_message(port, &msg) > 0) {
		...
		case SINK_READY:
			...
			msg.header = PD_HEADER(PD_DATA_REQUEST, 0, 0,
					       port->tx_msg_id, 1);
...
bool tcpc_pd_sink_check_charging(struct tcpc_port *port)
{
	...
	if (err || (state != TYPEC_STATE_SNK_POWER15 &&
		    ...
		    state != TYPEC_STATE_SNK_DEFAULT)) {
		tcpc_debug_log(port,
			       "TCPC wrong state for PD charging, err = %d, CC = 0x%x\n",
			       err, state);
		return false;
	}
	...
	if (!(valb & TCPC_POWER_STATUS_SINKING_VBUS)) {
		tcpc_debug_log(port,
			       "SINK VBUS is not enabled for dead battery\n");
		return false;
	}
...
static int tcpc_pd_sink_disable(struct tcpc_port *port)
{
	...
	if ((valb & TCPC_POWER_STATUS_VBUS_PRES) &&
	    (valb & TCPC_POWER_STATUS_SINKING_VBUS)) {
		dm_i2c_read(port->i2c_dev, TCPC_POWER_CTRL,
			    (uint8_t *)&valb, 1);
		valb &= ~TCPC_POWER_CTRL_AUTO_DISCH_DISCO;
		/* disable AutoDischargeDisconnect */
		dm_i2c_write(port->i2c_dev, TCPC_POWER_CTRL,
			     (const uint8_t *)&valb, 1);
...
static int tcpc_pd_sink_init(struct tcpc_port *port)
{
	...
	if (!(val & TCPC_ALERT_CC_STATUS)) {
		tcpc_debug_log(port,
			       "CC STATUS not detected for dead battery\n");
		return -EPERM;
	}
	...
	if (err || (state != TYPEC_STATE_SNK_POWER15 &&
		    ...
		    state != TYPEC_STATE_SNK_DEFAULT)) {
		tcpc_log(port,
			 "TCPC wrong state for dead battery, err = %d, CC = 0x%x\n",
			 err, state);
		return -EPERM;
	} else
	...
	/* As sink role */
	valb = 0x00;
	err = dm_i2c_write(port->i2c_dev, TCPC_MSG_HDR_INFO,
			   (const uint8_t *)&valb, 1);
	if (err) {
		tcpc_log(port, "%s dm_i2c_read failed, err %d\n",
			 __func__, err);
	...
	err = dm_i2c_write(port->i2c_dev, TCPC_RX_DETECT,
			   (const uint8_t *)&valb, 1);
	if (err) {
		tcpc_log(port, "%s dm_i2c_read failed, err %d\n",
			 __func__, err);
...
int tcpc_init(struct tcpc_port *port, struct tcpc_port_config config,
	      ss_mux_sel ss_sel_func)
{
	...
	ret = tcpc_polling_reg(port, TCPC_POWER_STATUS, 1,
			       TCPC_POWER_STATUS_UNINIT, 0, 1000);
	if (ret) {
		tcpc_log(port,
			 "%s: Polling TCPC POWER STATUS Initialization Status bit failed, ret = %d\n",
			 __func__, ret);
		return ret;
	}
	...
	valb = 0x80;
	ret = dm_i2c_write(port->i2c_dev, TCPC_FAULT_STATUS,
			   (const uint8_t *)&valb, 1);
	if (ret) {
		tcpc_log(port, "%s dm_i2c_read failed, err %d\n",
			 __func__, ret);
	...
	tcpc_log(port,
		 "TCPC:  Vendor ID [0x%x], Product ID [0x%x], Addr [I2C%u 0x%x]\n",
